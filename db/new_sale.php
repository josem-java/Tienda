<?php
	...
	$stmt = $conn->prepare("INSERT INTO tbl_venta (fecha_venta, total_venta) VALUES (:fecha,:total)");
	...
	$sql = "SELECT MAX(id) AS id from tbl_venta";
	$result = $conn->query($sql);
	$maxID = $result->fetch(PDO::FETCH_ASSOC);

	//Insertar en tbl_detalle_venta
	for($j = 0; $j < count($_POST['barCode']); $j++){
	$venDetalle = $conn->prepare("INSERT INTO tbl_detalle_venta (id_venta, id_articulo, cantidad) VALUES (:id_venta, :id_articulo, :cantidad)");
	...
	$upArt = $conn->prepare("UPDATE tbl_articulos SET cantidad_disponible = (cantidad_disponible - :cantidad_update) WHERE bar_code = :id_articulo_update");
